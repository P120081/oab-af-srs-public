...
#!/usr/bin/env python3
import numpy as np, pandas as pd, matplotlib.pyplot as plt, matplotlib.patheffects as path_effects
...
def volcano_plot(df: pd.DataFrame, out_png: str, title_drug: str|None=None):
    ...
    for cand in ["p_value","p-value","p value","P","P-value","P value"]:
        if cand in df.columns:
            rename[cand] = "p"; break
    for cand in ["χ^2","χ²","chi^2","Chi2","X2","x2"]:
        if cand in df.columns:
            rename[cand] = "chi2"; break
    ...
    if "p" in df.columns:
        df["p"] = (df["p"].astype(str).str.replace("<","",regex=False)
                              .str.replace(">","",regex=False)
                              .pipe(pd.to_numeric, errors="coerce"))
    if "p" not in df.columns:
        raise KeyError("Missing 'p' or equivalent ('p_value','p-value','P', ...). Columns found: " + ", ".join(map(str, df.columns)))
