...
def tto_distribution(df: pd.DataFrame, out_png: str, out_tif: str|None=None, y_max: float=730.0):
    ...
    eps = 1e-6
    y_vals = np.linspace(max(eps, y_min), y_max, 500)
    ...
    ax_pdf.grid(True)
    finite_pdf = pdf_vals[np.isfinite(pdf_vals)]
    xmax = float(finite_pdf.max()) * 1.05 if finite_pdf.size else 0.05
    ax_pdf.set_xlim(0, max(0.01, xmax))
