...
#!/usr/bin/env python3
"""
Forest plot (single-drug per DB sections).
Dual mode:
  (A) MSIP node: expects global 'table' to be defined.
  (B) CLI: --table CSV --out PNG [--tif TIF]
Inputs CSV minimal columns:
  DB, drug_of_interest, n11, ROR, ROR025, ROR975
Optional: p (or p-value), PRR025, chi2, IC025
"""
import os, argparse, numpy as np, pandas as pd, matplotlib.pyplot as plt, matplotlib.colors as mcolors
...
def forest(df: pd.DataFrame, out_png: str, out_tif: str|None=None):
    # Basic numeric coercions
    for c in ["n11","ROR","ROR025","ROR975","PRR025","IC025","chi2"]:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce")

    # Display strings
    df["ROR_CI_str"] = df.apply(lambda r: _fmt_ci(r.get("ROR"), r.get("ROR025"), r.get("ROR975")), axis=1)
    if "p" in df.columns:      df["p_str"]    = df["p"].apply(_fmt_p)
    ...
    for kind, payload in rows:
        ...
        if kind == "db":
            ...
            if n >= 3 and ic025 > 0:               _box(col_x["IC"],  (0,0.45,0.7,0.7), "white")
        y -= 1
...
def main():
    ...
    parser.add_argument("--table", required=False, help="CSV path (if absent, attempts MSIP 'table')")
    parser.add_argument("--out",   required=False, default="figure2_forest_plot.png")
    parser.add_argument("--tif",   required=False, default="figure2_forest_plot.tif")
    args = parser.parse_args()

    # Try MSIP 'table', fallback to CSV
    df = None
    ...
    forest(df, out_png=args.out, out_tif=args.tif)
